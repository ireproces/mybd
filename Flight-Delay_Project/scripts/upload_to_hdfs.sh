...
if hdfs dfs -test -d "${HDFS_TARGET_DIR}" 2>/dev/null; then
    echo -e "${RED}[!] The directory ${HDFS_TARGET_DIR} already exists on HDFS.${NC}"
    echo -e "[!] Recursively remove old data to avoid duplicates or conflicts..."
...
hdfs dfs -mkdir -p "${HDFS_TARGET_DIR}/test"
echo -e "${GREEN}[✓] HDFS structure created (complete, scalability, test).${NC}"

# Loading the sample dataset (flight_sample.parquet) into /test
if [ -e "${LOCAL_DATA_DIR}/flight_sample.parquet" ]; then
    echo -e "\\n${BLUE}[*] Loading the sample dataset (flight_sample.parquet)...${NC}"
    START_TIME=$(date +%s)
    hdfs dfs -put "${LOCAL_DATA_DIR}/flight_sample.parquet" "${HDFS_TARGET_DIR}/test/"
    END_TIME=$(date +%s)
    DURATION=$((END_TIME - START_TIME))
    echo -e "${GREEN}[✓] Sample dataset successfully loaded in ${DURATION}s.${NC}"
else
    echo -e "\\n${RED}[!] [ERROR] flight_sample.parquet not found in ${LOCAL_DATA_DIR}.${NC}"
fi

# Loading the base dataset (flight_100.parquet) into /complete
if [ -e "${LOCAL_DATA_DIR}/flight_100.parquet" ]; then
    ...
    echo -e "\\n${RED}[!] [ERROR] flight_100.parquet not found in ${LOCAL_DATA_DIR}.${NC}"
fi

# Cyclic loading of generated samples into /scalability
...
for SAMPLE in "${SAMPLES[@]}"; do
    ...
    if [ -e "${LOCAL_PATH}" ]; then
        ...
        echo -e "${RED}[!] [ERROR] ${SAMPLE}.parquet not found locally.${NC}"
    fi
done

# Final verification of the content uploaded to HDFS
echo -e "\\n${BLUE}[*] Final verification of the HDFS file system:${NC}"

echo -e "\\n${BLUE}[+] Contents of the 'test' folder:${NC}"
hdfs dfs -ls "${HDFS_TARGET_DIR}/test"

echo -e "\\n${BLUE}[+] Contents of the 'complete' folder:${NC}"
